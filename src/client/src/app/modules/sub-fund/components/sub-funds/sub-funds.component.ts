...
@Component({
  selector: 'app-sub-funds',
  templateUrl: './sub-funds.component.html',
  styleUrls: ['./sub-funds.component.scss']
})
export class SubFundsComponent {
  userName: string;
  userRoles = [];
  tableData: IEntitiesViewModel;
  controller: string = 'SubFunds';
  subFundsLoaded = false;
  ...
  constructor(
    ...
    private subFundsSevice: SubFundsService,
  ...
  private _fetchData() {
    this.subFundsSevice.all(this.userName).subscribe(data => {
      this.subFundsLoaded = true;
      this.tableData = data;
  ...
  update(model){
    this.subFundsLoaded = false;
    model.userName = this.userName;
    this.subFundsSevice.update(model).subscribe(data =>{
      this.subFundsLoaded = true;
      this.tableData = data;
      this._populateDataSource(data);
    })
  ...
  _getModel(command: string) {
    ...
    model.ControllerName = this.ToolBarComponent;
